...
===============================================================================
util.h stores a bunch of refactored code, so the following source code is 
really just a bunch of generic and probably non-related functions.
===============================================================================
 1. bsearch_ : binary searching functions
 2. hpsort_ : heap sorting functions
 3. comparsion functions consist of a bunch of function pointers to different
    operations, probably going to be deprecated or turn into macros.
 4. string functions consist of a bunch of functions that does basic parsing,
    honestly, the implementations are suboptimal.
=============================================================================*/
#include "util.h"
...
/* binary serach for integer or strings (case and non-case sensitive) */
int32_t bsearch_int(void * db, int32_t db_size, int32_t key, DBFIELD field, DBTYPE type) {
	...
	while(bottom <= top) {
		middle = (top + bottom) / 2;
		if(*(*type)(db, middle, field) == key)
			return middle;
		else if(*(*type)(db, middle, field) < key)
			bottom = middle + 1;
		else if(*(*type)(db, middle, field) > key)
			top = middle - 1;
	}
...
int32_t bsearch_str(void * db, int32_t db_size, char * key, DBFIELD_STR field, DBTYPE_STR type) {
	...
	while(bottom <= top) {
		middle = (top + bottom) / 2;
		if(strcmp((*type)(db, middle, field), key) == 0) {
			return middle;
		} else if(strcmp((*type)(db, middle, field), key) == -1)
			bottom = middle + 1;
		else if(strcmp((*type)(db, middle, field), key) == 1)
			top = middle - 1;
	}
...
int32_t bsearch_ncs_str(void * db, int32_t db_size, char * key, DBFIELD_STR field, DBTYPE_STR type) {
	...
	while(bottom <= top) {
		middle = (top + bottom) / 2;
		if(ncs_strcmp((*type)(db, middle, field), key) == 0) {
			return middle;
		} else if(ncs_strcmp((*type)(db, middle, field), key) == -1) {
			bottom = middle + 1;
		} else if(ncs_strcmp((*type)(db, middle, field), key) == 1)	{
			top = middle - 1;
		}
...
int32_t ncs_strcmp(const char * str1, const char * str2) {
   ...
   /* fatal error */
	...
	/* exit on the first mismatch in character */
	for(i = 0; str1[i+offset1] != '\0' || str2[i+offset2] != '\0'; i++) {
      /* skip the _ when comparing */
		if(str1[i+offset1] == '_') offset1++;
...
/* retrieve a substring of the string and stop on delimiter 
   the first one match only one delimiter,
   the second one match a set of delimiters.*/
...
/* WARNING: variable length argument list can fatally crash the program,
   be careful and review your variable length argument before continuing.
